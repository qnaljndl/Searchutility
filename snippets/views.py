...
from snippets.serializers import SnippetSerializer, UserSerializer, MapSearchSerializer, RequestSerializer
...
@api_view(['POST'])
def requestSearch(request, format=None):
    if request.method == 'POST':
        serializer = RequestSerializer(request.data)
        requestObject = serializer.create(request.data)
        value = requestObject.query
        payload = {'key': 'AIzaSyAcRq3Sd8Fq-ZKFxWW8AQxiiF_v0xm679s', 'input': value}
        response = requests.get('https://maps.googleapis.com/maps/api/place/autocomplete/json', params=payload)
        if serializer.is_valid():
            serializer.save()
